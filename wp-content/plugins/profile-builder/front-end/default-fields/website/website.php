...
function wppb_website_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
        ...
        $output =
			'<input class="text-input default_field_website" name="website" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="text" id="website" value="'.esc_url( wp_unslash( $input_value ) ).'" placeholder="'. esc_attr( strtoupper($item_title). $required_mark ) .'" />';
