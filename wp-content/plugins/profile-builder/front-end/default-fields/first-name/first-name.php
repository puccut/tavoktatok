...
function wppb_first_name_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){	
	...
	if ( $form_location != 'back_end' ){
        ...
        $output =
			'<input class="text-input default_field_firstname" name="first_name" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="text" id="first_name" value="'. esc_attr( wp_unslash( $input_value ) ) .'" placeholder="'. esc_attr( strtoupper($item_title). $required_mark ) .'" />';
