...
function wppb_username_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	...
	if ( $form_location != 'back_end' ){
        ...
        $output =
			'<input class="text-input default_field_username" name="username" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="text" id="username" value="'. esc_attr( $input_value ) .'"'.$readonly.'  placeholder="'. esc_attr( strtoupper($item_title). $required_mark ) .'" />';
