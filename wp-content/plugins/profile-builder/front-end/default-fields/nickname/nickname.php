...
function wppb_nickname_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
	...
	if ( $form_location != 'back_end' ){
        ...
        $output =
			'<input class="text-input default_field_nickname" name="nickname" maxlength="'. apply_filters( 'wppb_maximum_character_length', 70 ) .'" type="text" id="nickname" value="'. esc_attr( wp_unslash( $input_value ) ) .'"  placeholder="'. esc_attr( strtoupper($item_title). $required_mark ) .'" />';
